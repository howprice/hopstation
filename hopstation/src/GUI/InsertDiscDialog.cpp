...
void InsertDiscDialog::Update()
{
	std::lock_guard<std::mutex> lock(s_pendingLoadMutex);
	if (s_pendingLoad.pending)
	{
		CD& cd = Host::GetCD();
		if (cd.LoadFromFile(s_pendingLoad.path))
		{
			Host::GetBus().GetCDROM().InsertDisc(cd);
			LOG_INFO("Inserted disc: %s\n", cd.GetPath());
		}
		else
		{
			LOG_ERROR("Failed to load CD from file: %s\n", s_pendingLoad.path);
		}
...
bool InsertDiscDialog::ShowOpenFileDialog(SDL_Window* pWindow)
{
	{
		std::lock_guard<std::mutex> lock(s_pendingLoadMutex);
		if (s_pendingLoad.pending)
	...
	{
		{"Cue Sheets (*.cue)", "cue"},
//		{"Single-Track Raw Images (*.bin;*.img;*.iso)", "bin;img;iso"},
		{"Raw Binary Images (*.bin)", "bin"},
		{"All Files (*)", "*"}
	};
