...
SHADERCROSS=../../bin/macOS/shadercross
BIN=../../data/shaders
...
for filename in *.vert.hlsl; do
    if [ -f "$filename" ]; then
        echo "Compiling $filename to ${filename/.hlsl/.msl}"
        ${SHADERCROSS} "$filename" -s hlsl -d msl -t vertex -e "main" -o "${BIN}/${filename/.hlsl/.msl}"
    fi
done
...
for filename in *.frag.hlsl; do
    if [ -f "$filename" ]; then
        echo "Compiling $filename to ${filename/.hlsl/.msl}"
        ${SHADERCROSS} "$filename" -s hlsl -d msl -t fragment -e "main" -o "${BIN}/${filename/.hlsl/.msl}"
    fi
done
